...
{
  /// <summary>
  /// Tracks player's hacking progress across devices and networks
  /// </summary>
  public class PlayerProgressManager
  {
    ...
    /// <summary>
    /// Record that a device has been compromised
    /// </summary>
    public void RecordSystemCompromise(Device device, string exploitUsed, bool hasRoot)
    {
      ...
      string key = GetSystemKey(device.Hostname);

      if (!ownedSystems.ContainsKey(key))
      {
        // First time compromising this device
        var info = new OwnedSystemInfo
        {
          Hostname = device.Hostname,
          IPAddress = device.IPAddress,
          Type = device.DeviceType.Name, // Now uses DeviceType.Name
        ...
        if (!string.IsNullOrEmpty(exploitUsed))
        {
          ...
          AddDiscoveredVulnerability(device.Hostname, exploitUsed);
        }
        ...
        Debug.Log($"Device compromised: {device.Hostname} via {exploitUsed}");
      }
      else
      {
        ...
        if (hasRoot && !info.HasRootAccess)
        {
          ...
          Debug.Log($"Privilege escalation on {device.Hostname}: Root access obtained");
        }
